...
namespace LatticeUtils.Core.UnitTests
{
    public class TestCollectionUtils
    {
        ...
        public void AsReadOnly_LinkedList()
        {
            ...
            var readOnlyCollection = CollectionUtils.AsReadOnly<int>(collection);
            Assert.AreEqual(typeof(LatticeUtils.Core.Collections.ReadOnlyCollection<int>), readOnlyCollection.GetType());
            CollectionAssert.AreEqual(collection, readOnlyCollection);
        }
        ...
        {
            var inputReadOnlyCollection = new LatticeUtils.Core.Collections.ReadOnlyCollection<int>(new[] { 1, 2, 3 });
            var outputReadOnlyCollection = CollectionUtils.AsReadOnly<int>(inputReadOnlyCollection);
            Assert.AreSame(inputReadOnlyCollection, outputReadOnlyCollection);
        }
        ...
        {
            var enumerable = new[] { 1, 2, 3, 4, 5, 6, 7 };

            var pageList = CollectionUtils.Paginate(enumerable, pageSize: 3).ToList();
            Assert.AreEqual(3, pageList.Count);
            CollectionAssert.AreEqual(new[] { 1, 2, 3 }, pageList[0]);
            CollectionAssert.AreEqual(new[] { 4, 5, 6 }, pageList[1]);
            CollectionAssert.AreEqual(new[] { 7 }, pageList[2]);
        }

        [Test]
        public void PaginateIntoGroupings()
        {
            var enumerable = new[] { 1, 2, 3, 4, 5, 6, 7 };

            var pageList = CollectionUtils.PaginateIntoGroupings(enumerable, pageSize: 3).ToList();
            Assert.AreEqual(3, pageList.Count);
            Assert.AreEqual(0, pageList[0].Key);
            CollectionAssert.AreEqual(new[] { 1, 2, 3 }, pageList[0]);
            Assert.AreEqual(1, pageList[1].Key);
            CollectionAssert.AreEqual(new[] { 4, 5, 6 }, pageList[1]);
            Assert.AreEqual(2, pageList[2].Key);
            CollectionAssert.AreEqual(new[] { 7 }, pageList[2]);
        }

        [Test]
        public void Paginate_DeferredExecution()
        {
            var consumingEnumerableWrapper = new ConsumingEnumerableWrapper<int>(new[] { 1, 2, 3, 4, 5, 6, 7 });
            var consumingEnumerable = consumingEnumerableWrapper.AsConsumingEnumerable();

            var pages = CollectionUtils.Paginate(consumingEnumerable, pageSize: 3);

            var pageEnumerator = pages.GetEnumerator();

            Assert.IsTrue(pageEnumerator.MoveNext());
            CollectionAssert.AreEqual(new[] { 4, 5, 6, 7 }, consumingEnumerableWrapper.PeekRemaining());
            CollectionAssert.AreEqual(new[] { 1, 2, 3 }, pageEnumerator.Current);

            Assert.IsTrue(pageEnumerator.MoveNext());
            CollectionAssert.AreEqual(new[] { 7 }, consumingEnumerableWrapper.PeekRemaining());
            CollectionAssert.AreEqual(new[] { 4, 5, 6 }, pageEnumerator.Current);

            Assert.IsTrue(pageEnumerator.MoveNext());
            CollectionAssert.AreEqual(new int[0], consumingEnumerableWrapper.PeekRemaining());
            CollectionAssert.AreEqual(new[] { 7 }, pageEnumerator.Current);

            Assert.IsFalse(pageEnumerator.MoveNext());
        }
        ...
        private IEnumerable<T> CreateConsumingEnumerable<T>(IEnumerable<T> enumerable)
        {
            var q = new Queue<T>(enumerable);
            while (q.Any())
            {
        ...
        private class ConsumingEnumerableWrapper<T>
        {
            private readonly Queue<T> q;

            public ConsumingEnumerableWrapper(IEnumerable<T> enumerable)
            {
                q = new Queue<T>(enumerable);
            }

            public ICollection<T> PeekRemaining()
            {
                return q.ToList();
            }

            public IEnumerable<T> AsConsumingEnumerable()
            {
                while (q.Any())
                {
                    yield return q.Dequeue();
